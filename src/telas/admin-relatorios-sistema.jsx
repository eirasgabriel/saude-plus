...
import {
  CLINICAS_ADMINISTRATIVAS,
  CONSULTAS_SISTEMA,
  USUARIOS_SISTEMA,
  obterNomeClinica,
  obterRelatorioPorClinica,
  obterRelatorioPorEspecialidade,
  obterRelatorioPorStatus,
  obterResumoSistema,
} from "../dados/sistema-mock";
...
function AdminRelatoriosSistema() {
  ...
  const resumo = useMemo(
    () => obterResumoSistema(CLINICAS_ADMINISTRATIVAS, USUARIOS_SISTEMA, CONSULTAS_SISTEMA),
    []
  );
  const porClinica = useMemo(
    () => obterRelatorioPorClinica(CLINICAS_ADMINISTRATIVAS, CONSULTAS_SISTEMA),
    []
  );
  const porEspecialidade = useMemo(() => obterRelatorioPorEspecialidade(CONSULTAS_SISTEMA), []);
  const porStatus = useMemo(() => obterRelatorioPorStatus(CONSULTAS_SISTEMA), []);
  ...
  const consultasRecentes = useMemo(() => {
    return [...CONSULTAS_SISTEMA].sort((a, b) => {
              ...
              <tbody>
                {consultasRecentes.map((consulta) => (
                  <tr key={consulta.id} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-3 text-gray-600">
                      {obterNomeClinica(consulta.clinica_id)}
                    </td>
