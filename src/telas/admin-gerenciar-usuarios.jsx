...
function AdminGerenciarUsuarios() {
  ...
  const [usuarios, setUsuarios] = useState(USUARIOS_SISTEMA);
  ...
  function salvarUsuario(evento) {
    ...
    if (usuarioEditandoId) {
      ...
      const proximoId = Math.max(...usuarios.map((usuario) => usuario.id), 0) + 1;
      setUsuarios((listaAtual) => [
        {
          id: proximoId,
          nome: formulario.nome.trim(),
      ...
      setMensagem("Usuario criado no painel.");
    }
  ...
  function alternarStatus(usuarioId) {
    setUsuarios((listaAtual) =>
      listaAtual.map((usuario) =>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-5">
          ...
          >
            ...
            >
              ...
              {CLINICAS_ADMINISTRATIVAS.map((clinica) => (
                ...
                <tbody>
                  {usuariosFiltrados.map((usuario) => (
                    <tr key={usuario.id} className="border-b border-gray-100">
                      ...
                      <td className="px-3 py-3 text-gray-600">
                        {usuario.clinica_id
                          ? obterNomeClinica(usuario.clinica_id)
                          : "Sem vinculo"}
                      </td>
